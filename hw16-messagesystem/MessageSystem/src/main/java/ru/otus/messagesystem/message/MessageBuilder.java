...
public class MessageBuilder {
    ...
    public static <T, R extends ResultDataType> Message buildMessage(String from, String to, MessageId sourceMessageId,
                                                                     T data, MessageType msgType) {
        return buildMessage(from, to, sourceMessageId, data, msgType.getName(), null);
    }

    public static <R extends ResultDataType> Message buildReplyMessage(Message message, R data) {
        return buildMessage(message.getTo(), message.getFrom(), message.getId(), data,
                message.getType(), message.getCallbackId());
    }

    private static <T, R extends ResultDataType> Message buildMessage(String from, String to, MessageId sourceMessageId,
